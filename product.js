...
const makeRequest = async () => {
    try {
        ...
        let response = await localStorage.getItem("product");
...
const anotherFunc = async () => {
    ...
    for (i = 0; i < response.data.length; i++) {
        ...
        //console.log(response.data[i].imageUrl);
        createHTML(i, response.data[i].name, response.data[i].last_name, response.data[i].description, response.data[i].imageUrl);
    }
    displayUser();
}
...
const createHTML = function (i, name, last_name, description, imageUrl) {
    //console.log(response.data[i].description);
    //console.log(response.data[i].name);
    //console.log(response.data[i].last_name);
    //console.log(response.data[i].imageUrl);
    ...
    html = html + '  <img class="card-img-top" src="' + imageUrl + '" alt="' + name + ' ' + last_name + '">'
    html = html + '  <div class="card-body">'
    html = html + '    <p class="card-text"><strong>' + name + ' ' + last_name + '</strong><br><em>' + description + '</em></p>'
    html = html + '  </div>'
    html = html + '</div>'
    if (i == 2 || i == 5) {
        //card.innerHTML = html;
